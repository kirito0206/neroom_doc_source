...
开通 IM 安全通服务后，即可实现消息安全通审核。当发送的消息匹配命中敏感词后，会触发 [`NERoomListener.onAntiSpamMessageIntercepted`](https://doc.yunxin.163.com/neroom/references/android/dokka/Latest/zh/html/com/netease/yunxin/kit/roomkit/api/NERoomListener.html#onAntiSpamMessageIntercepted(java.lang.String,com.netease.yunxin.kit.roomkit.api.model.NEYidunAntiSpamRes)) 回调。回调中的 `antiSpamRes` 字段为具体的安全通审核结果。

审核结果（[`NEYidunAntiSpamRes`](https://doc.yunxin.163.com/neroom/references/android/dokka/Latest/zh/html/com/netease/yunxin/kit/roomkit/api/model/NEYidunAntiSpamRes.html)）详情如下：

参数 | 类型 | 说明
--- | --- | --- 
`code` | Integer | 内容审核状态码。<ul><li> 200：内容审核结果返回正常</li><li>404：返回的内容审核结果为空</li><li>414：返回的内容审核结果过长</li></ul>
