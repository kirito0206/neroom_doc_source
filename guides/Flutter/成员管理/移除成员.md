...
2. 调用 `kickMemberOut` 接口将成员从房间移除。
    ...
    ```dart   
    Future<VoidResult> kickMemberOut(String userUuid);
    ```
...
roomContext?.kickMemberOut(userUuid).then((result){
    if(result.isSuccess()){
      debugPrint('kickMemberOut success');
    }else{
      debugPrint('kickMemberOut error code:${result.code},msg:${result.msg}');
    }
});
```
...
| [`kickMemberOut`](https://pub.dev/documentation/netease_roomkit/latest/netease_roomkit/NERoomContext/kickMemberOut.html) | 踢出成员 |